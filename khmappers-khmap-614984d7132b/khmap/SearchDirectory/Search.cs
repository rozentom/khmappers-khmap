...
namespace khmap.SearchDirectory
{
    public class Search : ISearch
    {
        ...
        public IEnumerable<Group> searchGroups(string text, ObjectId userId)
        {
            return _isg.SearchGroupsFunc(text, userId);
        }
        public IEnumerable<ApplicationUser> searchUsers(string text, ObjectId userId)
        {
            return _isu.SearchUsersFunc(text, userId);
        }
        public IEnumerable<Map> searchMaps(string text, ObjectId userId)
        {
            return _ism.searchMapsFunc(text, userId);
        }
        ...
        public Dictionary<ISearchType, Object> searchFunc(string text, ObjectId userId, List<ISearchType> typesIncluded)
        {
            ...
            foreach(ISearchType st in _searchTypes)
            {
                ...
                if (searchThisType)
                {
                    var result = st.search(text, userId);
